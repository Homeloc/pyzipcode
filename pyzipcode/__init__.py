...
ZIP_FIND_QUERY_FR = "SELECT * FROM ZipCodes WHERE city LIKE ?"
...
def format_result_fr(zips):
    if len(zips) > 0:
        return [ZipCodeFR(zip) for zip in zips]
    else:
        return None
...
class ZipCodeDatabaseFR(ZipCodeDatabase):
    def get(self, zip):
        return format_result_fr(self.conn_manager.query(ZIP_QUERY, [zip]))
    ...
    def find_zip(self, city=None):
        ...
        return format_result(self.conn_manager.query(ZIP_FIND_QUERY_FR, [city]))

    def find_ads(self, zip):
        zip = self[zip]
        return zip.ad1, zip.ad2
